...
export default function ProfessorSecurityPage() {
    ...
    <div>
      ...
      {facePhoto && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 24, maxWidth: 440, width: '90%' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16 }}><i className="fas fa-camera" style={{ marginRight: 8 }} />Photo de référence</h3>
                {facePhoto.student_name && <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--text-muted)' }}>{facePhoto.student_name} — {facePhoto.exam_title}</p>}
              </div>
            ...
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10, textAlign: 'center' }}>Tentative #{facePhoto.attemptId}</p>
          </div>
        </div>
      )}
    </div>
