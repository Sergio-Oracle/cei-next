    ...
    <div>
      ...
      <div className="page-header" style={{ marginBottom: 28 }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize:17 }}>Correction individuelle par IA ou traitement en lot de plusieurs copies</p>
        </div>
      </div>
      ...
      {noSubjects && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '48px 32px', textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontWeight: 700, fontSize:19, color: 'var(--text)', marginBottom: 8 }}>Aucun sujet disponible</div>
          <div style={{ fontSize:15.5, color: 'var(--text-muted)', marginBottom: 20 }}>
          ...
          <Link href="/dashboard/professor/create-subject"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: 'var(--primary)', color: 'white', borderRadius: 8, fontWeight: 600, fontSize:17, textDecoration: 'none' }}>
          ...
          <div style={{ display: 'flex', gap: 0, marginBottom: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderBottom: 'none', borderRadius: '14px 14px 0 0', overflow: 'hidden' }}>
            {(['single', 'batch'] as Tab[]).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                style={{
                  flex: 1, padding: '14px 18px', border: 'none', fontWeight: 600, fontSize:17, cursor: 'pointer',
            ...
            {activeTab === 'single' && (
              <form onSubmit={handleSingle} id="single-correction-form">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div>
                    <label style={{ display: 'block', fontWeight: 600, fontSize:15.5, marginBottom: 7, color: 'var(--text)' }}>
                    ...
                    <select value={singleSubjectId} onChange={e => setSingleSubjectId(e.target.value)} required
                      style={{ width: '100%', padding: '10px 14px', border: '1.5px solid var(--border)', borderRadius: 8, fontSize:17, color: 'var(--text)', background: 'var(--surface)', outline: 'none', boxSizing: 'border-box' }}>
                  ...
                  <div>
                    ...
                    <div
                      ...
                      {singleFile ? (
                        <>
                          ...
                          <div style={{ fontWeight: 600, fontSize:17, color: '#15803d', marginBottom: 4 }}>{singleFile.name}</div>
                          <div style={{ fontSize:14.5, color: '#64748b' }}>{(singleFile.size / 1024).toFixed(0)} Ko</div>
                        </>
                      ) : (
                        <>
                          <i className="fas fa-cloud-upload-alt" style={{ fontSize: 35, color: '#94a3b8', display: 'block', marginBottom: 8 }} />
                          <div style={{ fontWeight: 600, fontSize:17, color: 'var(--text)', marginBottom: 4 }}>Glissez-déposez le fichier ici</div>
                          <div style={{ fontSize:15.5, color: 'var(--text-muted)' }}>ou cliquez pour parcourir</div>
                        </>
                      )}
                    </div>
                    ...
                    {singleFile && (
                      <button type="button" onClick={() => { setSingleFile(null); if (singleInputRef.current) singleInputRef.current.value = '' }}
                        style={{ marginTop: 8, background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize:15.5, fontWeight: 600 }}>
                  ...
                  {singleError && (
                    <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '12px 16px', color: '#991b1b', fontSize:15.5, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <i className="fas fa-circle-exclamation" />{singleError}
                    </div>
                  )}

                  <button type="submit" disabled={singleSubmitting}
                    style={{ padding: '12px 24px', background: singleSubmitting ? '#93c5fd' : '#2563eb', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize:18, cursor: singleSubmitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                    {singleSubmitting
              ...
              <form onSubmit={handleBatch} id="batch-correction-form">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div
                      ...
                      {batchFiles && batchFiles.length > 0 ? (
                        <>
                          ...
                          <div style={{ fontWeight: 600, fontSize:17, color: '#15803d', marginBottom: 4 }}>{batchFiles.length} fichier(s) sélectionné(s)</div>
                          <div style={{ fontSize:14.5, color: '#64748b' }}>Cliquez pour changer la sélection</div>
                        </>
                      ) : (
                        <>
                          <i className="fas fa-folder-open" style={{ fontSize: 35, color: '#94a3b8', display: 'block', marginBottom: 8 }} />
                          <div style={{ fontWeight: 600, fontSize:17, color: 'var(--text)', marginBottom: 4 }}>Sélectionnez un dossier de copies</div>
                          <div style={{ fontSize:15.5, color: 'var(--text-muted)' }}>Ou glissez-déposez plusieurs fichiers ici</div>
                        </>
                      )}
                    </div>
                  ...
                  <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', padding: '12px 14px', background: '#f8fafc', border: '1px solid var(--border)', borderRadius: 8 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, fontSize:15.5, color: 'var(--text)' }}>Extraction automatique des noms</div>
                      <div style={{ fontSize:14.5, color: 'var(--text-muted)', marginTop: 2 }}>
                  ...
                  {batchSubmitting && (
                    <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '12px 16px', color: '#1d4ed8', fontSize:15.5, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button type="submit" disabled={batchSubmitting}
                    style={{ padding: '12px 24px', background: batchSubmitting ? '#93c5fd' : '#2563eb', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize:18, cursor: batchSubmitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                    {batchSubmitting
                      ? <><i className="fas fa-spinner fa-spin" />Correction en cours…</>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, fontSize:17 }}>Comment ça marche ?</span>
            </div>
            {[
              ...
              <div key={step.n} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                <div style={{ width: 24, height: 24, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize:13, fontWeight: 700, color: '#2563eb' }}>
                  {step.n}
                </div>
                <div style={{ fontSize:15.5, color: 'var(--text)', lineHeight: 1.5, paddingTop: 2 }}>{step.text}</div>
              </div>
            ))}
          </div>
          ...
          {/* Mode guide */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
            <div style={{ fontWeight: 700, fontSize:15.5, color: 'var(--text)', marginBottom: 12 }}>
            ...
            <div style={{ padding: '10px 12px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, marginBottom: 8 }}>
              <div style={{ fontWeight: 600, fontSize:14.5, color: '#1d4ed8', marginBottom: 4 }}>
            ...
            <div style={{ padding: '10px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8 }}>
              <div style={{ fontWeight: 600, fontSize:14.5, color: '#15803d', marginBottom: 4 }}>
          ...
          <div style={{ background: '#fefce8', border: '1px solid #fde68a', borderRadius: 14, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize:15.5, color: '#92400e', marginBottom: 4 }}>À savoir</div>
                <div style={{ fontSize:14.5, color: '#78350f', lineHeight: 1.6 }}>
          ...
          <div style={{ background: 'var(--surface)', border: `1px solid var(--border)`, borderRadius: 14, overflow: 'hidden', borderTop: `3px solid ${singleResult.score >= 10 ? '#10b981' : '#ef4444'}` }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize:19, color: 'var(--text)' }}>Correction terminée</div>
                  <div style={{ fontSize:15.5, color: 'var(--text-muted)', marginTop: 2 }}>{singleResult.subject_title}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  style={{ padding: '8px 16px', background: '#2563eb', color: 'white', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer', fontSize:15.5, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <i className="fas fa-file-pdf" />Télécharger PDF
                </button>
                <button onClick={() => setSingleResult(null)}
                  style={{ padding: '8px 12px', background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize:17 }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 24 }}>
                <div style={{ padding: 14, background: '#f8fafc', borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize:13, color: '#64748b', textTransform: 'uppercase', letterSpacing: .5, marginBottom: 4 }}>Étudiant</div>
                  <div style={{ fontWeight: 700, fontSize:18, color: 'var(--text)' }}>{singleResult.student_name}</div>
                </div>
                <div style={{ padding: 14, background: singleResult.score >= 10 ? '#dcfce7' : '#fee2e2', borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize:13, color: singleResult.score >= 10 ? '#15803d' : '#991b1b', textTransform: 'uppercase', letterSpacing: .5, marginBottom: 4, fontWeight: 600 }}>Note</div>
                  <div style={{ fontWeight: 800, fontSize:31, color: singleResult.score >= 10 ? '#10b981' : '#ef4444', lineHeight: 1 }}>
                    {fmtScore(singleResult.score)}<span style={{ fontSize:15.5, fontWeight: 500 }}>/20</span>
                  </div>
                </div>
                <div style={{ padding: 14, background: '#f8fafc', borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize:13, color: '#64748b', textTransform: 'uppercase', letterSpacing: .5, marginBottom: 4 }}>Mention</div>
                  <div style={{ fontWeight: 700, fontSize:18, color: 'var(--text)' }}>{getMention(singleResult.score)}</div>
                </div>
              </div>
              {singleResult.feedback && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    <span style={{ fontWeight: 700, fontSize:17 }}>Feedback détaillé de l'IA</span>
                  </div>
                  <div style={{ padding: '14px 16px', background: '#f8fafc', borderRadius: 8, fontSize:15.5, lineHeight: 1.8, color: '#334155', maxHeight: 320, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>
                    {singleResult.feedback}
                  </div>
                </div>
              )}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', borderTop: '3px solid #10b981' }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize:19, color: 'var(--text)' }}>Correction en lot terminée</div>
                  <div style={{ fontSize:15.5, color: 'var(--text-muted)', marginTop: 2 }}>{(batchResult.corrected + batchResult.errors)} copie(s) traitée(s)</div>
                </div>
              </div>
              <button onClick={() => setBatchResult(null)}
                style={{ padding: '8px 14px', background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize:15.5, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <div className="grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 20 }}>
                <div style={{ padding: 14, background: '#dcfce7', borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize:13, color: '#15803d', textTransform: 'uppercase', letterSpacing: .5, fontWeight: 600, marginBottom: 2 }}>Corrigées</div>
                  <div style={{ fontWeight: 800, fontSize:28.5, color: '#15803d' }}>{batchResult.corrected}</div>
                </div>
                {batchResult.results && batchResult.results.filter(r => typeof r.score === 'number').length > 0 ? (
                  <div style={{ padding: 14, background: '#eff6ff', borderRadius: 10, textAlign: 'center' }}>
                    <div style={{ fontSize:13, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: .5, fontWeight: 600, marginBottom: 2 }}>Moyenne</div>
                    <div style={{ fontWeight: 800, fontSize:28.5, color: '#1d4ed8' }}>
                      ...
                      <span style={{ fontSize:15.5, fontWeight: 500 }}>/20</span>
                    </div>
                  </div>
                ) : <div />}
                <div style={{ padding: 14, background: batchResult.errors > 0 ? '#fee2e2' : '#f8fafc', borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize:13, color: batchResult.errors > 0 ? '#991b1b' : '#64748b', textTransform: 'uppercase', letterSpacing: .5, fontWeight: 600, marginBottom: 2 }}>Erreurs</div>
                  <div style={{ fontWeight: 800, fontSize:28.5, color: batchResult.errors > 0 ? '#ef4444' : '#94a3b8' }}>{batchResult.errors}</div>
                </div>
              </div>
                    ...
                    <thead>
                      <tr style={{ background: '#f8fafc' }}>
                        {['Fichier','Étudiant','Note','Statut'].map(h => (
                          <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize:14.5, color: '#64748b', fontWeight: 700, borderBottom: '1px solid var(--border)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {batchResult.results.map((r, i) => (
                        <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                          <td style={{ padding: '10px 14px', fontSize:15.5, color: '#475569', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.filename}</td>
                          <td style={{ padding: '10px 14px', fontSize:15.5, fontWeight: 600, color: 'var(--text)' }}>{r.student_name || '—'}</td>
                          <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                            {typeof r.score === 'number'
                              ? <span style={{ fontWeight: 700, fontSize:18, color: r.score >= 10 ? '#10b981' : '#ef4444' }}>{fmtScore(r.score)}/20</span>
                              : <span style={{ color: '#94a3b8', fontSize:15.5 }}>—</span>}
                          </td>
                          <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                            {r.error
                              ? <span style={{ background: '#fee2e2', color: '#991b1b', padding: '2px 8px', borderRadius: 99, fontSize:13, fontWeight: 600 }}>Erreur</span>
                              : <span style={{ background: '#dcfce7', color: '#15803d', padding: '2px 8px', borderRadius: 99, fontSize:13, fontWeight: 600 }}>Corrigée</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {batchResult.errors > 0 && batchResult.error_details && batchResult.error_details.length > 0 && (
                <div style={{ marginTop: 16, padding: 14, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8 }}>
                  <div style={{ fontWeight: 600, fontSize:15.5, color: '#991b1b', marginBottom: 8 }}>
                    <i className="fas fa-triangle-exclamation" style={{ marginRight: 6 }} />{batchResult.errors} erreur(s) rencontrée(s)
                  </div>
                  <ul style={{ margin: 0, paddingLeft: 20, fontSize:14.5, color: '#7f1d1d', lineHeight: 1.8 }}>
