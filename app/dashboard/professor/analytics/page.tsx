    ...
    <div>
      <div className="page-header" style={{ marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', margin: '4px 0 0', fontSize:17, display: 'flex', flexWrap: 'wrap', gap: '0 14px' }}>
        ...
        <>
          ...
          {(data.total_corrected ?? 0) > 0 && (
            <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
                <p style={{ margin: 0, fontSize:15.5, fontWeight: 700, color: 'var(--text)', alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <div style={{ position: 'relative', width: 140, height: 140, borderRadius: '50%',
                  ...
                  <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)',
                    ...
                    <span style={{ fontSize:24, fontWeight: 800, color: passRate >= 50 ? '#10b981' : '#ef4444' }}>{passRate}%</span>
                    <span style={{ fontSize:11, color: '#94a3b8', textTransform: 'uppercase' }}>réussite</span>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 14 }}>
                  <span style={{ fontSize:14.5, color: '#475569', display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 20px' }}>
                <p style={{ margin: '0 0 14px', fontSize:15.5, fontWeight: 700, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                ...
                {(data.top_exams ?? []).length === 0
                  ? <p style={{ color: '#94a3b8', fontSize:14.5 }}>Min. 2 copies corrigées requises</p>
                ...
                {(data.bottom_exams ?? []).length === 0
                  ...
                  : <table style={{ width: '100%', borderCollapse: 'collapse' }}><tbody>
                    {(data.bottom_exams ?? []).map((exam, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '8px 10px', fontSize:15.5 }}><i className="fas fa-arrow-down" style={{ color: '#ef4444', marginRight: 6, fontSize: 12 }} />{exam.title}</td>
                        <td style={{ padding: '8px 10px', fontWeight: 700, fontSize:15.5, color: sc(exam.avg_score), whiteSpace: 'nowrap' }}>{fmtScore(exam.avg_score)}/20</td>
                        <td style={{ padding: '8px 10px', fontSize:14.5, color: '#64748b', whiteSpace: 'nowrap' }}>{exam.pass_rate}%</td>
                      </tr>
                    ))}
                ...
                <tbody>
                  {(data.recent_corrections ?? []).map((c, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '7px 10px', fontSize:15.5, fontWeight: 700, color: sc(c.score) }}>
          ...
          {!data.total_exams && (
            <div className="card" style={{ textAlign: 'center', padding: 60 }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize:15.5, margin: 0 }}>Créez des examens en ligne et corrigez des copies pour voir les statistiques.</p>
            </div>
          )}
        </>
      )}
