...
export default function ProfessorProctorGroupsPage() {
  ...
  // Retour : "Cette partie doit aussi être chez les Professeurs" — mêmes
  // routes backend que la page admin, mais désormais scopées côté serveur :
  // un professeur ne voit/gère que SES groupes (ProctorGroup.created_by_id)
  // et ne peut rattacher un groupe qu'à un EC dont il a la charge
  // (ECAssignment). /api/ecs et /api/users/proctors sont déjà conçus pour
  // être appelés par un professeur (auto-scopés côté serveur, pas de
  // filtrage supplémentaire nécessaire ici).
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [groupsRes, proctorsRes, supervisorsRes, ecsRes] = await Promise.all([
        api.get<Group[]>('/api/admin/proctor_groups'),
        api.get<Surveillant[]>('/api/users/proctors'),
        api.get<Superviseur[]>('/api/users/proctors?role=superviseur'),
        api.get<any>('/api/ecs'),
      ])
      setGroups(Array.isArray(groupsRes) ? groupsRes : [])
      setSurveillants(Array.isArray(proctorsRes) ? proctorsRes : [])
      setSuperviseurs(Array.isArray(supervisorsRes) ? supervisorsRes : [])
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {superviseurs.length === 0 && (
                <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '-12px 0 0' }}>
                  Aucun compte superviseur — un administrateur doit d'abord en créer un depuis « Utilisateurs ».
                </p>
              )}
              ...
              <div>
                ...
                {availableEcs.length === 0 && ecs.length === 0 && (
                  <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Aucun EC ne vous est encore assigné — voir « Affectations EC ».</p>
                )}
              </div>
            </div>
