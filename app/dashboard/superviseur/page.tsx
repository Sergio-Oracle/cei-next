    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize:24, fontWeight: 700, color: 'var(--text)' }}>Tableau de bord Superviseur</h2>
      ...
      {callRequests.length > 0 && (
        <div style={{ background: '#fffbeb', border: '1.5px solid #fbbf24', borderRadius: 16, padding: '16px 20px', marginBottom: 24 }}>
          <h3 style={{ margin: '0 0 12px', fontSize:17, fontWeight: 700, color: '#92400e', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {callRequests.map(r => (
              <div key={r.attempt_id} style={{ background: 'white', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 160 }}>
                  <div style={{ fontWeight: 700, fontSize:15.5, color: '#1e293b' }}>{r.student_name}</div>
                  <div style={{ fontSize:14.5, color: '#64748b' }}>{r.exam_title} — {new Date(r.timestamp).toLocaleTimeString('fr-FR')}</div>
                </div>
                <button onClick={() => setActiveCall(r)} style={{ fontSize:14.5, fontWeight: 700, color: 'white', background: '#d97706', border: 'none', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', textAlign: 'center', padding: '64px 24px' }}>
          ...
          <h3 style={{ color: '#475569', fontSize:21.5, fontWeight: 600, margin: '0 0 8px' }}>Aucun groupe assigné</h3>
          <p style={{ color: '#94a3b8', fontSize:17, margin: 0 }}>Un administrateur doit vous rattacher à un ou plusieurs groupes de surveillants.</p>
        </div>
      ) : (
          ...
          {data.groups.map(group => (
            <div key={group.id} style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--border)', overflow: 'hidden' }}>
              <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                <h3 style={{ margin: 0, fontSize:18, fontWeight: 700, color: 'var(--text)' }}>{group.name}</h3>
                <span style={{ fontSize:14.5, color: 'var(--text-muted)' }}>— {group.members.length} surveillant(s)</span>
                <span style={{ marginLeft: 'auto', fontSize:13, fontWeight: 700, padding: '3px 10px', borderRadius: 12, background: '#f1f5f9', color: '#475569' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#f8fafc' }}>
                        <th style={{ padding: '8px 24px', textAlign: 'left', fontSize:13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: '#64748b' }}>Surveillant</th>
                        <th style={{ padding: '8px 24px', textAlign: 'right', fontSize:13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: '#64748b' }}>Statut</th>
                        <th style={{ padding: '8px 24px', textAlign: 'right', fontSize:13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: '#64748b' }}></th>
                      </tr>
                    </thead>
                    <tbody>
                      {group.members.map(m => (
                        <tr key={m.id} style={{ borderTop: '1px solid var(--border)' }}>
                          <td style={{ padding: '10px 24px' }}>
                            <div style={{ fontWeight: 500, fontSize:17, color: 'var(--text)' }}>{m.full_name}</div>
                            <div style={{ fontSize:14.5, color: 'var(--text-muted)' }}>{m.email}</div>
                          </td>
                          <td style={{ padding: '10px 24px', textAlign: 'right' }}>
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize:14.5, fontWeight: 600, padding: '4px 12px', borderRadius: 20, background: STATUS_META[m.status].bg, color: STATUS_META[m.status].color }}>
                          ...
                          <td style={{ padding: '10px 24px', textAlign: 'right' }}>
                            {m.id != null && m.full_name && (
                              <button onClick={() => setProctorCall({ id: m.id as number, name: m.full_name as string })}
                                ...
                                style={{ background: '#f0fdf4', color: '#166534', border: '1px solid #bbf7d0', borderRadius: 8, padding: '6px 12px', fontSize:14.5, fontWeight: 600, cursor: 'pointer' }}>
