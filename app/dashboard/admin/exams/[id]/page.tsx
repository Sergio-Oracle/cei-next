...
function SBadge({ s }: { s: string }) {
  ...
  return <span style={{ background: c + '20', color: c, fontSize:13, fontWeight: 700, padding: '2px 8px', borderRadius: 20, border: `1px solid ${c}40` }}>{l}</span>
}
...
export default function AdminExamDetailPage() {
  ...
  function onRescheduleStartChange(v: string) {
    const oldStart = parseLocalInput(rescheduleForm.start_time)
    const oldEnd = parseLocalInput(rescheduleForm.end_time)
  ...
  function onRescheduleEndChange(v: string) {
    setRescheduleForm(f => ({ ...f, end_time: v }))
  }
  ...
  // que l'enseignant/admin n'a pas cliqué ici. Simple confirmation, sans mot
  // de passe : action réservée aux enseignants/admins.
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {(exam.status === 'draft' || exam.status === 'scheduled') && <button className="btn btn-secondary" onClick={openCompositionModal} title="Pagination et ordre aléatoire des questions"><i className="fas fa-random" /> Composition</button>}
          ...
          >
            {publishing
              ...
              : exam.results_published
                ? <><i className="fas fa-eye-slash" /> Dépublier les résultats</>
                : <><i className="fas fa-bullhorn" /> Publier les résultats</>}
          </button>
        ...
        <div className="card">
          ...
          <div style={{ padding: '18px 24px' }}>
            {[
              ...
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none', fontSize:17 }}>
        ...
        <div className="card">
          ...
          <div style={{ padding: '20px 24px', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[
              ...
              <span key={String(label)} style={{ padding: '5px 12px', borderRadius: 99, fontSize:15, fontWeight: 500, background: ok ? '#dcfce7' : 'var(--background)', color: ok ? '#166534' : 'var(--text-muted)', border: `1px solid ${ok ? '#bbf7d0' : 'var(--border)'}` }}>
                ...
                : attempts.map(a => (
                  <tr key={a.id}>
                    <td>
                      ...
                      {a.student_email && <div style={{ fontSize:13, color: 'var(--text-muted)' }}>{a.student_email}</div>}
                      {(a.extra_minutes ?? 0) > 0 && <span style={{ fontSize:12, background: '#dbeafe', color: '#1d4ed8', padding: '1px 6px', borderRadius: 10, fontWeight: 700 }}>+{a.extra_minutes}min</span>}
                    </td>
                    <td><SBadge s={a.status} /></td>
                    <td style={{ fontSize:14.5 }}>{fmt(a.started_at)}</td>
                    <td style={{ fontSize:14.5 }}>{a.submitted_at ? fmt(a.submitted_at) : '—'}</td>
                    <td>
                      {a.score != null
                        ? <strong style={{ color: a.score >= 10 ? '#10b981' : '#ef4444' }}>{fmtScore(a.score)}/20</strong>
                        : <span style={{ color: 'var(--text-muted)', fontSize:14.5 }}>—</span>}
                    </td>
                    <td>
                      {a.risk_score != null
                        ? <span style={{ fontWeight: 700, fontSize:14.5, color: a.risk_score >= 70 ? '#ef4444' : a.risk_score >= 40 ? '#f59e0b' : '#10b981' }}>{a.risk_score}%</span>
                        : '—'}
                    </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', borderRadius: 14, padding: 28, width: 420, maxWidth: '95vw' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize:15.5, fontWeight: 600, display: 'block', marginBottom: 6 }}>Note (0–20)</label>
              <input type="number" min={0} max={20} step={0.5} value={gradeForm.score}
                onChange={e => setGradeForm(f => ({ ...f, score: e.target.value }))}
                style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid var(--border)', fontSize:17, background: 'var(--surface)', color: 'var(--text)', boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize:15.5, fontWeight: 600, display: 'block', marginBottom: 6 }}>Commentaire (optionnel)</label>
              <textarea value={gradeForm.feedback} onChange={e => setGradeForm(f => ({ ...f, feedback: e.target.value }))} rows={3}
                style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid var(--border)', fontSize:17, background: 'var(--surface)', color: 'var(--text)', boxSizing: 'border-box', resize: 'vertical' }} />
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', borderRadius: 14, padding: 28, width: 420, maxWidth: '95vw' }}>
            ...
            <div style={{ marginBottom: 20, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              <input type="checkbox" id="comp_randomize_admin" checked={compositionForm.randomize_questions}
                onChange={e => setCompositionForm(f => ({ ...f, randomize_questions: e.target.checked }))}
                style={{ width: 'auto', marginTop: 3, flexShrink: 0 }} />
              <div>
                <label htmlFor="comp_randomize_admin" style={{ fontSize:15.5, fontWeight: 600, cursor: 'pointer' }}>Ordre des questions aléatoire</label>
