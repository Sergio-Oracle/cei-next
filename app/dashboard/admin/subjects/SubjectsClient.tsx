...
export default function SubjectsClient({ initialSubjects }: Props) {
  ...
  async function handleDelete(id: number) {
    if (!confirm('Supprimer ce sujet et toutes ses dépendances ?')) return
    try {
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p>Consultez, prévisualisez et gérez tous les sujets créés sur la plateforme</p>
        </div>
      </div>
      ...
      <div className="grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 24 }}>
        <button
          onClick={() => router.push('/dashboard/admin/create-subject')}
        ...
        <button
          onClick={() => router.push('/dashboard/admin/suggestions')}
          style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '18px 22px', background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 14, cursor: 'pointer', textAlign: 'left', transition: 'border-color .15s, box-shadow .15s' }}
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontWeight: 600, fontSize:17 }}>
              {subjects.length} sujet{subjects.length !== 1 ? 's' : ''} au total
            </span>
          </div>
            ...
            <tbody>
              {subjects.length === 0 ? (
                  ...
                  <td colSpan={7}>
                    <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                        <button className="btn btn-secondary" onClick={() => router.push('/dashboard/admin/create-subject')}>
                          <i className="fas fa-upload" /> Créer un sujet
                        </button>
                        <button className="btn btn-primary" onClick={() => router.push('/dashboard/admin/suggestions')}>
                ...
                const dateStr  = s.created_at
                  ...
                  : '—'
                const initials = (s.creator_name ?? 'NA').split(' ').map((w: string) => w[0]).slice(0, 2).join('').toUpperCase()
                    ...
                    <td>
                      {s.creator_name ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#dbeafe', color: '#1d4ed8', fontSize:12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{initials}</div>
                          <span style={{ fontSize:15.5 }}>{s.creator_name}</span>
                        </div>
                      ) : <span style={{ color: 'var(--text-muted)', fontSize:15.5 }}>—</span>}
                    </td>
      ...
      {(previewSubject || previewLoading) && (
        <div onClick={e => { if (e.target === e.currentTarget) setPreviewSubject(null) }}
                ...
                <div style={{ padding: '16px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      {previewSubject.creator_name && (
                        <span style={{ fontSize:14.5, color: 'var(--text-muted)' }}>
                          <i className="fas fa-user" style={{ marginRight: 4 }} />{previewSubject.creator_name}
                        </span>
                      )}
