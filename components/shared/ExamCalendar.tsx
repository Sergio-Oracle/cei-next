            ...
            return (
              <div key={exam.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderBottom: i < filtered.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <div style={{ width: 56, height: 56, borderRadius: 12, background: exam.status === 'active' ? '#dcfce7' : 'var(--background)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: `2px solid ${exam.status === 'active' ? '#10b981' : 'var(--border)'}` }}>
                  {start ? (<><div style={{ fontWeight: 700, fontSize:24, lineHeight: 1 }}>{start.getDate()}</div><div style={{ fontSize:12, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{MONTHS_FR[start.getMonth()].slice(0,3)}</div></>) : <i className="fas fa-calendar" />}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize:18 }}>{exam.title}</div>
                  {exam.subject_title && <div style={{ fontSize:14.5, color: 'var(--text-muted)' }}>{exam.subject_title}</div>}
                  <div style={{ fontSize:14.5, color: 'var(--text-muted)', marginTop: 3 }}>
                ...
                <span style={{ fontSize:14.5, fontWeight: 600, padding: '4px 10px', borderRadius: 99, background: st.bg, color: st.color }}>{st.label}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
          <div className="card">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <h3 style={{ margin: 0, fontSize:19, fontWeight: 700 }}>{MONTHS_FR[mon]} {year}</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 8 }}>
              {DAYS_FR.map(d => <div key={d} style={{ textAlign: 'center', fontSize:13, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', padding: '4px 0' }}>{d}</div>)}
            </div>
              ...
              {cells.map((cell, i) => {
                ...
                return (
                  <div key={i} onClick={() => setSelected(isSel ? null : cell)}
                    ...
                    <div style={{ fontSize:15.5, fontWeight: isToday ? 700 : 400, color: isToday ? '#2563eb' : 'var(--text)', marginBottom: 3 }}>{cell.getDate()}</div>
                    {dayExams.slice(0,2).map(e => (
                      <div key={e.id} style={{ fontSize:11, background: STATUS_STYLE[e.status]?.bg ?? '#f1f5f9', color: STATUS_STYLE[e.status]?.color ?? '#94a3b8', borderRadius: 3, padding: '1px 4px', marginBottom: 1, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontWeight: 600 }}>
                        {e.title}
                      </div>
                    ))}
                    {dayExams.length > 2 && <div style={{ fontSize:11, color: 'var(--text-muted)', fontWeight: 600 }}>+{dayExams.length - 2} autre{dayExams.length - 2 > 1 ? 's' : ''}</div>}
                  </div>
                )
              })}
            </div>
          ...
          <div className="card">
            {!selected ? (
              <div style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize:15.5 }}>Cliquez sur un jour pour voir les examens</p>
              </div>
            ) : (
              <>
                <h4 style={{ marginBottom: 16, fontSize:17, fontWeight: 700 }}>
                ...
                {selectedExams.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 24, color: 'var(--text-muted)', fontSize:15.5 }}>Aucun examen ce jour</div>
                ) : selectedExams.map(e => {
                  ...
                  return (
                    <div key={e.id} style={{ padding: '12px 14px', borderRadius: 10, background: 'var(--background)', border: '1px solid var(--border)', marginBottom: 10 }}>
                      <div style={{ fontWeight: 600, fontSize:17, marginBottom: 6 }}>{e.title}</div>
                      <span style={{ fontSize:13, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: st.bg, color: st.color, display: 'inline-block', marginBottom: 8 }}>{st.label}</span>
                      <div style={{ fontSize:14.5, color: 'var(--text-muted)', lineHeight: 1.7 }}>
                      ...
                      {e.status === 'active' && (
                        <Link href={`/proctor/${e.id}`} className="btn btn-success" style={{ marginTop: 10, fontSize:14.5, padding: '5px 12px', display: 'inline-flex' }}>
                          <i className="fas fa-eye" /> Surveiller
                        </Link>
                      )}
                    </div>
