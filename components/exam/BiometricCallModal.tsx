...
/**
 * Appel vocal/vidéo étudiant → surveillant/superviseur/professeur, repli
 * après plusieurs échecs de reconnaissance faciale à l'accès examen (avant
 * même la création d'une tentative — donc pas de room `private-{attemptId}`
 * disponible, contrairement à CallSurveillantModal.tsx dont ce composant est
 * calqué). Room dédiée `biometric-{studentId}-{examId}`, voir
 * routes/biometric.py `biometric_private_token`.
 */
...
export default function BiometricCallModal({ examId, studentId, examTitle, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        await api.post('/api/biometric/fallback/call_request', { exam_id: examId })
      } catch { /* non bloquant */ }
      ...
      try {
        ...
        room.on(LK.RoomEvent.ParticipantConnected, () => { setStatus('connected'); success('Un membre du staff a rejoint l\'appel') })
  ...
  const statusLabel: Record<typeof status, string> = {
    ...
    waiting: 'En attente qu\'un membre du staff réponde…',
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.75)', zIndex: 9500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#0f172a', borderRadius: 16, width: '100%', maxWidth: 420, overflow: 'hidden', border: '1px solid rgba(255,255,255,.1)', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <i className="fas fa-user-shield" style={{ color: '#f59e0b' }} />
          <div>
            <div style={{ color: 'white', fontWeight: 700, fontSize: 17 }}>Appel — vérification d'identité</div>
