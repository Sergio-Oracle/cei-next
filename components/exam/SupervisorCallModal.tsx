...
/**
 * Appel vocal/vidéo superviseur → surveillant, depuis le tableau de bord
 * superviseur — pour échanger directement si un surveillant ne remplit pas
 * son rôle plutôt que de simplement le sanctionner sans discussion. Réutilise
 * le même schéma LiveKit privé que les appels étudiant ↔ surveillant, mais
 * sur une room dédiée par surveillant (supcall-{proctorId}), indépendante de
 * tout examen en cours.
 */
...
export default function SupervisorCallModal({ proctorId, proctorName, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        // Notifie le surveillant (badge + toast/modal entrant) avant de
        // rejoindre la room, pour qu'il puisse répondre même s'il rejoint après nous.
        await api.post(`/api/superviseur/proctor_call/${proctorId}/request`, {})
      } catch { /* non bloquant */ }
      ...
      try {
        ...
        room.on(LK.RoomEvent.ParticipantConnected, () => { setStatus('connected'); success(`${proctorName} a rejoint l'appel`) })
        room.on(LK.RoomEvent.Disconnected, () => { if (!cancelled) setStatus('error') })
        // L'autre partie a raccroché — l'appel se termine des deux côtés,
        // comme un vrai appel téléphonique, au lieu de rester "connecté" côté
        // appelant avec une vidéo distante figée.
        ...
        if (!cancelled) { setStatus('error'); toastErr(e.message || "Impossible de démarrer l'appel") }
      }
    }
  ...
  const statusLabel: Record<typeof status, string> = {
    ...
    waiting: `En attente que ${proctorName} réponde…`,
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.75)', zIndex: 9500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#0f172a', borderRadius: 16, width: '100%', maxWidth: 420, overflow: 'hidden', border: '1px solid rgba(255,255,255,.1)', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ color: 'white', fontWeight: 700, fontSize: 14 }}>Appel — {proctorName}</div>
            <div style={{ color: 'rgba(255,255,255,.5)', fontSize: 11 }}>Surveillant</div>
          </div>
        </div>
        ...
        <div style={{ position: 'relative', background: '#000', aspectRatio: '4/3', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* Toujours monté dans le DOM (visibilité gérée en CSS) — sinon
              t.attach(localVidRef.current) juste avant setCamOn(true) échoue
              silencieusement car l'élément n'existe pas encore au moment de
              l'appel (camOn était encore false lors de ce rendu). */}
