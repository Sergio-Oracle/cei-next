...
/**
 * Appel vocal/vidéo étudiant → surveillant/professeur, depuis le tableau de
 * bord (hors page d'examen) — utilisé pour obtenir un code de reprise à
 * usage unique après une déconnexion. Réutilise la même room LiveKit privée
 * (`private-{attemptId}`) que celle déjà utilisée dans le sens surveillant →
 * étudiant pendant l'examen (voir app/proctor/monitor/[id] et app/exam/[id]).
 */
...
export default function CallSurveillantModal({ attemptId, examTitle, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        // Signale la demande d'appel (notifie le/les surveillants assignés,
        // ou le professeur à défaut) avant de rejoindre la room — ainsi
        // l'appel apparaît côté surveillant même s'il rejoint après nous.
        await api.post(`/api/exam_attempts/${attemptId}/call_request`, {})
      } catch { /* non bloquant : on tente quand même de rejoindre la room */ }
      ...
      try {
        ...
        room.on(LK.RoomEvent.ParticipantConnected, () => { setStatus('connected'); success('Votre surveillant a rejoint l\'appel') })
        ...
        try {
          ...
          setMicOn(true)
        } catch { /* micro optionnel — l'appel reste utilisable sans */ }
      } catch (e: any) {
  ...
  const statusLabel: Record<typeof status, string> = {
    ...
    waiting: 'En attente qu\'un surveillant réponde…',
